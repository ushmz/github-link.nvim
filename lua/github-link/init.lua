...
---@return string | nil ref The ref name or nil if error
local function get_ref(which_ref)
	...
	elseif which_ref == "file" then
		local ref = vim.fn.systemlist("git rev-list -1 HEAD -- " .. vim.fn.shellescape(vim.fn.expand("%")))[1]
		return ref
	else
		return nil
	end
...
function M.get_commit_link(which_ref)
	...
	local ref = get_ref(which_ref)
	if not ref then
		vim.api.nvim_err_writeln("[github-link.nvim] Unknown ref type: " .. which_ref)
		return
	end
